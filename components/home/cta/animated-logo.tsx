    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          // Reset for re-animation if triggerOnce is false
    ...
    >
      <g transform="translate(0.000000,347.000000) scale(0.100000,-0.100000)" fill="currentColor" stroke="none">
        ...
        >
          {shouldAnimate && (
            <>
              <animate attributeName="stroke-dashoffset" from="1" to="0" dur={toSec(drawMs)} begin="0.08s" fill="freeze" />
              <animate
                attributeName="fill-opacity"
                begin={toSec(drawMs)}
                from="0"
        ...
        >
          {shouldAnimate && (
            <>
              <animate attributeName="stroke-dashoffset" from="1" to="0" dur={toSec(drawMs)} begin="0.16s" fill="freeze" />
              <animate
                attributeName="fill-opacity"
                begin={toSec(drawMs)}
                from="0"
      ...
      {/* Optional: subtle entrance fade for the whole logo */}
      <defs />
      {shouldAnimate && (
        <g>
