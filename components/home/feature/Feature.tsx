import { Copy, Zap } from "lucide-react";
import MusicCardThemeCustomizer from "./Customize";
import { Browser } from "./Browser";
import { CustomPointer } from "./CustomPointer";

export default function AnimatedComponentsShowcase() {
  return (
    <div className="w-full py-20 overflow-hidden px-6 xl:px-22 xl:container mx-auto">
      <div className="relative z-10 xl:container mx-auto px-4 sm:px-6 md:px-8 lg:px-8 py-8 sm:py-12 lg:py-16 bg-card border border-border/60 rounded-2xl">
        <div className="block lg:hidden space-y-12 sm:space-y-16">
          {/* Left Section - Copy/Paste Components */}
          <div className="space-y-6 sm:space-y-8">
            <div className="flex items-center gap-2 text-muted-foreground text-xs sm:text-sm">
              <Copy className="w-3 h-3 sm:w-4 sm:h-4" />
              <span>Copy/Paste Components</span>
            </div>

            <div className="space-y-4 sm:space-y-6">
              <h1 className="text-3xl md:text-4xl font-bold leading-tight text-foreground">
                Polished{" "}
                <span className="relative inline-block tracking-tight">
                  <span className="relative z-0 border border-border/60 bg-muted rounded px-1.5 py-0.5 font-mono text-sm">
                    &lt;Component/&gt;
                  </span>
                </span>{" "}
                <span className="block sm:inline">
                  for elegant UIs and clean layouts
                </span>
              </h1>
            </div>

            <div className="space-y-4 relative w-full overflow-hidden">
              <Browser
                url="https://nyxui.com/"
                className="w-full h-auto max-w-full"
              >
                <CustomPointer />
              </Browser>
            </div>
          </div>

          {/* Separator for mobile/tablet */}
          <div className="w-full h-px bg-border/60"></div>

          {/* Right Section - Development Speed */}
          <div className="space-y-6 sm:space-y-8">
            <div className="flex items-center gap-2 text-muted-foreground text-xs sm:text-sm">
              <Zap className="w-3 h-3 sm:w-4 sm:h-4" />
              <span>Development Speed</span>
            </div>

            <div className="space-y-4 sm:space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold leading-tight text-foreground">
                <span className="relative inline-block tracking-tight">
                  <span className="relative z-0 border border-border/60 bg-muted rounded px-1.5 py-0.5">
                    Customize
                  </span>
                </span>{" "}
                <span className="block sm:inline">
                  with ease. Launch with speed.
                </span>
              </h2>
            </div>

            <div className="w-full overflow-hidden">
              <MusicCardThemeCustomizer />
            </div>
          </div>
        </div>

        {/* Desktop Layout (side by side) */}
        <div className="lg:grid lg:grid-cols-2 gap-16 items-start relative hidden">
          {/* Left Section - Copy/Paste Components */}
          <div className="space-y-8">
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              <Copy className="w-3 h-3" />
              <span>Copy/Paste Components</span>
            </div>
            <div className="space-y-4">
              <h1 className="text-4xl font-bold leading-tight text-foreground">
                Polished{" "}
                <span className="relative inline-block tracking-tight">
                  <span className="relative z-0 border border-border/60 bg-muted rounded px-1.5 py-0.5 font-mono text-base">
                    Components
                  </span>
                </span>{" "}
                for elegant UIs and clean layouts
              </h1>
            </div>
            <div className="space-y-4 relative">
              <Browser
                url="https://nyxui.com/"
                className="w-full h-auto max-w-full"
              >
                <CustomPointer />
              </Browser>
            </div>
          </div>

          {/* Right Section - Development Speed */}
          <div className="space-y-8">
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              <Zap className="w-3 h-3" />
              <span>Development Speed</span>
            </div>
            <div className="space-y-5">
              <h2 className="text-4xl font-bold leading-tight text-foreground">
                <span className="relative inline-block tracking-tight">
                  <span className="relative z-0 border border-border/60 bg-muted rounded px-1.5 py-0.5">
                    Customize
                  </span>
                </span>{" "}
                with ease. Launch with speed.
              </h2>
            </div>
            <div className="mt-2">
              <MusicCardThemeCustomizer />
            </div>
          </div>
        </div>
        <div className="hidden lg:block h-full w-px bg-border/60 absolute top-0 left-1/2 transform -translate-x-1/2"></div>
      </div>
    </div>
  );
}
