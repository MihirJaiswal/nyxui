...
export function MorphingBlob({
    ...
    () => ({
      ...
      custom: {
        gradient: customColors
          ...
          : "",
        glow: customColors
          ? `shadow-[${customColors.from}]/60`
          : "shadow-blue-500/60",
        filter: `drop-shadow(0 0 8px ${
          customColors?.from || "rgba(59, 130, 246, 0.5)"
        })`,
      },
    }),
    [customColors]
  ...
  const generateBlobPath = useCallback(
    (
      factor: { points: number; variance: number; tension: number },
      hover = false,
      click = false
    ) => {
      ...
      for (let i = 0; i < points; i++) {
        ...
        const cp1x =
          current.x +
          (next.x - blobPoints[(i - 1 + points) % points].x) * tension;
        const cp1y =
          current.y +
          (next.y - blobPoints[(i - 1 + points) % points].y) * tension;
        const cp2x =
          next.x - (blobPoints[(i + 2) % points].x - current.x) * tension;
        const cp2y =
          next.y - (blobPoints[(i + 2) % points].y - current.y) * tension;
