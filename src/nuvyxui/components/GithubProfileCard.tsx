...
export const themes: ThemeOption[] = [
  {
    ...
    cardShadow: "shadow-lg shadow-gray-200/50 hover:shadow-xl hover:shadow-gray-200/60",
  ...
  },
  {
    id: "gradient-blue",
    name: "Gradient Blue",
    cardBg: "bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950",
  ...
  },
  {
    ...
    cardBg: "bg-gradient-to-br from-purple-50 to-fuchsia-50 dark:from-purple-950 dark:to-fuchsia-950",
  ...
  },
  {
    ...
    cardShadow: "shadow-lg shadow-black/40 hover:shadow-xl hover:shadow-black/50",
...
export function GitHubProfileCard({
  ...
  const [profile, setProfile] = useState<ProfileData | null>(manualMode ? profileData || null : null)
  ...
  const fetchProfileData = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all(reposData.map(async (repo: GitHubRepo) => {
        ...
        <div className="flex flex-col items-start gap-3">
          <div className="flex items-start w-full justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className={`text-lg font-bold leading-tight truncate ${currentTheme.textPrimary}`}>
                  {profile.name}
                </h2>
          ...
          {profile.bio && <p className={`my-2 text-xs line-clamp-2 ${currentTheme.textSecondary}`}>{profile.bio}</p>}
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-xs">
          {profile.location && (
            <div className={`flex items-center gap-1 ${currentTheme.textSecondary}`}>
  ...
  const ProfileStats = useMemo(() => {
    ...
    return (
      <div className="px-4 md:max-w-md">
        <div className={`flex justify-center gap-4 sm:gap-8 md:gap-20 px-2 sm:px-6 md:px-12 py-2.5 ${currentTheme.statsBg} rounded-md`}>
          <div className="flex flex-col">
            <span className={`text-sm sm:text-base text-center font-semibold ${currentTheme.textPrimary}`}>
              {profile.followers.toLocaleString()}
            </span>
            <span className={`text-xs text-center ${currentTheme.textSecondary}`}>followers</span>
          </div>
          <div className="flex flex-col">
            <span className={`text-sm sm:text-base text-center font-semibold ${currentTheme.textPrimary}`}>
              {profile.following.toLocaleString()}
            </span>
            <span className={`text-xs text-center ${currentTheme.textSecondary}`}>following</span>
          </div>
          <div className="flex flex-col">
            <span className={`text-sm sm:text-base text-center font-semibold ${currentTheme.textPrimary}`}>
              {profile.publicRepos.toLocaleString()}
            </span>
            <span className={`text-xs text-center ${currentTheme.textSecondary}`}>repositories</span>
          </div>
        </div>
      </div>
    );
  ...
  const ActivityGraph = useMemo(() => {
    if (!profile?.contributionData || profile.contributionData.length === 0) return null;

    return (
      <div className="px-4 pt-3">
        <h3 className={`text-xs font-medium uppercase mb-1.5 ${currentTheme.textSecondary}`}>Activity</h3>
        <div
          ...
          <svg className="h-full w-full" viewBox="0 0 100 30" preserveAspectRatio="none">
            <polyline
              points={profile.contributionData
                .map(
                  (value, index) => `${index * (100 / (profile.contributionData?.length || 1))},${30 - value * 30}`,
                )
                .join(" ")}
              fill="none"
              stroke="currentColor"
            ...
            <path
              d={`M0,30 ${profile.contributionData
                .map(
                  (value, index) => `L${index * (100 / (profile.contributionData?.length || 1))},${30 - value * 30}`,
                )
                .join(" ")} L100,30 Z`}
              fill="currentColor"
  ...
  const Languages = useMemo(() => {
    ...
    return (
      <div className="px-4 py-3">
        <h3 className={`text-xs font-medium uppercase mb-1.5 ${currentTheme.textSecondary}`}>Top Languages</h3>
              ...
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-1.5">
                  <div
                    ...
                    aria-hidden="true"
                  />
                  <span className={`font-medium ${currentTheme.textPrimary}`}>{item.name}</span>
                </div>
                <span className={`font-medium ${currentTheme.textPrimary}`}>{item.percentage}%</span>
              </div>
              <div
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <div className="px-4 md:max-w-md">
          <div className={`flex justify-center gap-4 sm:gap-8 md:gap-20 px-2 sm:px-6 md:px-12 py-4 ${currentTheme.statsBg} rounded-md`}>
            {[1, 2, 3].map((item) => (
        ...
        <div className="px-4 pt-3">
          ...
          <div className={`h-[36px] w-full rounded-md ${currentTheme.badgeBg} animate-pulse`} />
        </div>
        <div className="px-4 py-3">
        ...
        <div className={`flex items-center justify-between px-4 py-2 ${currentTheme.cardBorder.includes("border-") ? currentTheme.cardBorder.split(" ")[1] : "border-gray-100"}`}>
    ...
    return (
      ...
      >
        <div className="p-6 flex flex-col items-center justify-center">
          ...
          <h3 className={`text-lg font-semibold ${currentTheme.textPrimary} mb-1 text-center`}>
            Profile Not Found
          </h3>
          <p className={`text-sm ${currentTheme.textSecondary} text-center mb-4`}>
            {error.includes("404") ? 
