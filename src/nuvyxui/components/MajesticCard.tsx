...
export function MajesticCard({
  ...
  const themeStyles = {
    ...
    cosmic: {
      background:
        "bg-gradient-to-br from-purple-900 via-violet-800 to-blue-900 text-white",
  ...
  const getBorderStyles = useCallback(() => {
    ...
    switch (borderStyle) {
      ...
      case "glow":
        return `border ${
          currentTheme.border
        } shadow-[0_0_10px_3px_rgba(255,255,255,0.3)] shadow-${
          theme === "neon" ? "indigo" : theme === "cosmic" ? "purple" : "blue"
        }-500/30`;
      case "dashed":
  ...
  const getShadowStyles = useCallback(() => {
    ...
    switch (shadowType) {
      ...
      case "glow":
        return isHovered
          ? `shadow-[0_0_20px_5px] ${currentTheme.glow}`
          : `shadow-[0_0_10px_2px] ${currentTheme.glow}`;
      default:
        return `${sizes[shadowSize]} ${currentTheme.shadow}`;
    }
  }, [
  ...
  useEffect(() => {
    if (variant === "layered" && cardRef.current) {
      const layerElements = Array.from(
        cardRef.current.querySelectorAll("[data-layer]")
      );
      ...
      if (variant === "tilt" || variant === "morph" || variant === "breathe") {
        ...
        layersCopy.forEach((layer) => {
          const depth = Number.parseFloat(
            layer.getAttribute("data-layer") || "1"
          );
  ...
  const motionProps: HTMLMotionProps<"div"> = {
    ...
    style: {
      x:
        variant === "float" || variant === "wave"
          ? springFloatX
          : scrollEffect
          ? springY
          : springX,
      y: variant === "float" || variant === "wave" ? springFloatY : springY,
      rotate: variant === "float" || variant === "wave" ? springRotate : 0,
              ...
              <div
                ...
                style={{
                  ...
                  transform: `translateY(${
                    Math.sin(waveTime * speed + delay) * 5
                  }px) 
                            scaleY(${
                              1 + Math.sin(waveTime * speed * 0.5) * 0.2
                            })`,
                  bottom: `${i * 12}px`,
                }}
              />
