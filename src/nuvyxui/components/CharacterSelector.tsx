...
export type CharacterSelectorProps = {
  ...
  selectionIndicatorPosition?: "topLeft" | "topRight" | "bottomLeft" | "bottomRight"
...
export function CharacterSelector({
  ...
  const displayedCharacters = useMemo(() => {
    return lazyLoad ? filteredCharacters.slice(0, visibleItems) : filteredCharacters
  ...
  const getCardContainerStyle = (): CSSProperties => {
    if (displayMode === "flex") {
      ...
      return {
        display: "grid",
        gridTemplateColumns: `repeat(auto-fill, minmax(${typeof cardWidth === "number" ? `${cardWidth}px` : cardWidth}, 1fr))`,
        gap: typeof cardGap === "number" ? `${cardGap}px` : cardGap,
  ...
  const getHoverEffectClass = () => {
    switch (hoverEffect) {
      ...
      case "glow": return "hover:shadow-lg hover:shadow-primary/30"
            ...
            <ScrollArea className="h-[400px] p-2">
              <AnimatePresence>
                <motion.div
                  ...
                  animate="animate"
                  exit="exit"
                  transition={{ duration: animationDuration / 1000, staggerChildren: 0.05 }}
                >
                  <CharacterGrid
          ...
          {showDialogFooter && (
            <DialogFooter className="flex justify-between sm:justify-between pt-4 border-t border-muted/10">
              ...
              <div className="flex gap-3">
                ...
                >
                  {confirmButtonText} {showSelectionCount && selectedCharacters.length > 0 && `(${selectedCharacters.length})`}
                </Button>
              </div>
            </DialogFooter>
          )}
...
function CharacterCard({
  ...
  const cardStyle: CSSProperties = {
    ...
    borderRadius: typeof cardBorderRadius === "number" ? `${cardBorderRadius}px` : cardBorderRadius,
    padding: typeof cardPadding === "number" ? `${cardPadding}px` : cardPadding,
    aspectRatio: cardAspectRatio,
    backgroundColor: backgroundColor || undefined,
    borderColor: borderColor || undefined,
    borderStyle: enableCardBorder ? (emptyCardStyle === "dashed" && !character ? "dashed" : "solid") : "none",
    ...
    if (!character) {
      switch (emptyCardStyle) {
        ...
        case "highlighted":
          return "bg-muted/70";
        case "custom":
          return backgroundColor ? "" : "bg-gradient-to-br from-muted/20 to-muted/10";
        default:
            ...
            <div className="w-full h-full border border-gray-500">
              <div className="w-full h-full flex flex-col items-center justify-center">
                ...
                <div className="w-full h-1 relative overflow-hidden">
                  ...
                  <h1 className="text-white font-medium text-sm text-center py-3">{character.name}</h1>
                </div>
              </div>
            </div>
          ) : (
...
function CharacterGrid({
  ...
  const getIndicatorPosition = (position: string) => {
    switch (position) {
      case "topLeft": return "top-2 left-2";
            ...
            {showName(isSelected) && (
              <div
                className={cn(
                  ...
                  nameVisibility === "hover" && !isSelected ? "opacity-0 hover:opacity-100" : "opacity-100",
                  "transition-opacity"
                )}
                style={{ color: textColor || undefined }}
              />
            )}
            {isSelected && (selectedEffect === "checkmark" || selectedEffect === "multiple") && (
              <div 
                className={cn(
                ...
                style={{ 
                  width: typeof selectionIndicatorSize === "number" ? `${selectionIndicatorSize}px` : selectionIndicatorSize,
                  height: typeof selectionIndicatorSize === "number" ? `${selectionIndicatorSize}px` : selectionIndicatorSize
                }}
              >
      ...
      <DialogContent className="sm:max-w-md rounded-xl border-0 shadow-lg">
        <DialogHeader className="space-y-2 pb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <DialogDescription className="text-gray-600 dark:text-gray-300">
            ...
            {selectedCharacters.length !== 1 ? "s" : ""}. Are you sure you want to proceed?
          </DialogDescription>
        </DialogHeader>
