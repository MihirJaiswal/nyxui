...
export type MorphingButtonProps = {
  variant?: "expand" | "collapse" | "rotate" | "skew" | "liquid" | "pulse" | "bounce";
  size?: "xs" | "sm" | "md" | "lg" | "xl";
...
export const MorphingButton = ({
  ...
  const iconContent = useMemo(() => {
    if (loading) {
      return (
        <span className={iconPosition === "only" ? "" : iconPosition === "left" ? "mr-2" : "ml-2"}>
          <LoadingSpinner />
        </span>
      );
    }
    ...
    const spacingClass = iconPosition === "only"
      ? ""
      : iconPosition === "left" ? "mr-2" : "ml-2";

    return (
      <span className={`transition-transform duration-300 ${isHovered && !disabled ? "scale-110" : ""} ${spacingClass}`}>
        {icon}
      </span>
    );
  ...
  const handleMouseEvents = disabled || loading 
    ? {} 
    : {
