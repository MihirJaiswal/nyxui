...
import { useState, createContext, useContext, useEffect, ReactNode, useRef } from 'react';
import {
  motion,
  MotionValue,
  useMotionValue,
...
function ImageSlider({
  ...
  const handleInteraction = (event: React.MouseEvent | React.TouchEvent) => {
    ...
    if (contentDimensions) {
      ...
      if (clientX < interactiveLeft || clientX > interactiveLeft + interactiveWidth ||
          clientY < interactiveTop || clientY > interactiveTop + interactiveHeight) {
        return; 
      }
    }
    ...
    >
      <div
        ...
        className={cn(
          ...
          hoverControl && (orientation === 'horizontal' ? 'cursor-ew-resize' : 'cursor-ns-resize'),
          className
        )}
        ...
        style={{ 
          ...
          width: contentDimensions ? `${contentDimensions.width}px` : undefined,
          height: contentDimensions ? `${contentDimensions.height}px` : undefined,
          display: 'flex',
      ...
      >
        <div 
          ...
          style={{
            ...
            height: contentDimensions ? `${contentDimensions.height}px` : '100%',
          }}
        >
          {children}
        </div>
...
const ImageLayer = ({
  ...
  const secondLayerClip = useTransform(
    motionPosition,
    (value) => orientation === 'horizontal'
    ...
    >
      <div 
        ...
        style={{
          ...
          width: orientation === 'horizontal' ? `${width + hitAreaSize}px` : '100%',
          height: orientation === 'vertical' ? `${width + hitAreaSize}px` : '100%',
