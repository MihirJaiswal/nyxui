...
export const MorphingButton = ({
  ...
  const shadowClasses = {
    ...
    inner: "shadow-inner",
    glow: `shadow-lg ${
      color === "primary" ? "shadow-blue-400/40" :
      color === "secondary" ? "shadow-purple-400/40" :
      color === "success" ? "shadow-green-400/40" :
      color === "danger" ? "shadow-red-400/40" :
      color === "warning" ? "shadow-yellow-400/40" :
      color === "info" ? "shadow-cyan-400/40" :
      color === "violet" ? "shadow-violet-400/40" :
      color === "indigo" ? "shadow-indigo-400/40" :
      color === "teal" ? "shadow-teal-400/40" :
  ...
  const colorClasses = {
    primary: "bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white border border-blue-600",
    secondary: "bg-gradient-to-br from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white border border-purple-600",
    success: "bg-gradient-to-br from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white border border-emerald-600",
    danger: "bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white border border-red-600",
    warning: "bg-gradient-to-br from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-white border border-amber-500",
    ...
    slate: "bg-gradient-to-br from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 text-white border border-slate-700",
    violet: "bg-gradient-to-br from-violet-500 to-violet-600 hover:from-violet-600 hover:to-violet-700 text-white border border-violet-600",
    indigo: "bg-gradient-to-br from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white border border-indigo-600",
  ...
  const getIconContent = () => {
    if (!icon) return null;
    const spacingClass = iconPosition === "only" ? "" : iconPosition === "left" ? "mr-2" : "ml-2";
    return (
      <span className={`transition-transform duration-300 ${isHovered ? "scale-110" : ""} ${spacingClass}`}>
        {icon}
      </span>
    );
  };
    ...
    switch (variant) {
      ...
      case "gradient":
        ...
        return `transition-all duration-500 bg-gradient-to-r 
                ...
                color === "info" ? "cyan-300" : 
                color === "violet" ? "violet-400" : 
                color === "indigo" ? "indigo-400" : 
                color === "teal" ? "teal-400" : 
                color === "rose" ? "rose-400" : 
                color === "amber" ? "amber-400" : 
                color === "slate" ? "slate-500" : "gray-600"}  
                via-${color === "primary" ? "blue-500" : 
      ...
      case "glow":
        ...
        return `transition-all duration-300 
                ${isHovered ? `shadow-lg shadow-${
                  ...
                  color === "secondary" ? "purple-400/50" :
                  color === "success" ? "emerald-400/50" :
                  color === "danger" ? "red-400/50" :
                  color === "warning" ? "amber-400/50" :
                  color === "info" ? "cyan-400/50" :
                  color === "violet" ? "violet-400/50" :
                  color === "indigo" ? "indigo-400/50" :
                  color === "teal" ? "teal-400/50" :
                  color === "rose" ? "rose-400/50" :
                  color === "amber" ? "amber-400/50" :
                  color === "slate" ? "slate-400/50" :
                  "gray-400/50"
                }` : ""} 
                ${isActive ? "scale-95" : "scale-100"}`;
      case "pulse":
