...
const Text = () => {
  ...
  onMouseEnter={(e) => {
    ...
    const wordRotate = e.currentTarget.querySelector('.word-rotate-container');
    if (wordRotate) {
...
>
  <style jsx>{`
    @keyframes shake {
      ...
      2%, 6%, 10%, 14%, 18%, 22%, 26%, 30%, 34%, 38%, 42%, 46% { transform: translateX(-8px); }
      4%, 8%, 12%, 16%, 20%, 24%, 28%, 32%, 36%, 40%, 44% { transform: translateX(8px); }
      50%, 60%, 70% { transform: translateX(4px); }
      55%, 65%, 75% { transform: translateX(-4px); }
      80%, 90% { transform: translateX(2px); }
      85%, 95% { transform: translateX(-2px); }
    }
