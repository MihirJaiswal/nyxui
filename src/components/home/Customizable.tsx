...
const SpotifyCard = ({ theme, children, rounded, shadow }: SpotifyCardProps) => {
  ...
  return <div className={getThemeStyles()}>{children}</div>
}
...
const MusicCardThemeCustomizer = () => {
  ...
  const generateCode = () => `<SpotifyCard theme="${theme}" shadow rounded="xl">
  <div className="overflow-hidden">
    <div className="w-full h-64 relative">
      <Image src="/cover.png" alt="Album artwork" width={400} height={256} className="w-full h-full object-cover" />
    ...
    <div className="p-6 flex flex-col">
      ...
      <div className="mb-5">
        <div className="relative h-1.5 bg-gray-700 rounded-full overflow-hidden cursor-pointer">
          <div className="absolute top-0 left-0 h-full bg-green-500 rounded-full" style={{ width: '20%' }}></div>
        </div>
  ...
  const slideVariants = {
    ...
    center: {
      x: 0,
      opacity: 1,
      transition: { type: 'spring', stiffness: 350, damping: 30, mass: 1, duration: 0.4 }
    },
    exitRight: { x: 300, opacity: 0, transition: { type: 'tween', ease: 'easeInOut', duration: 0.3 } },
    exitLeft: { x: -300, opacity: 0, transition: { type: 'tween', ease: 'easeInOut', duration: 0.3 } }
  }
        ...
        <div className="md:hidden">
                ...
                <AnimatePresence mode="wait">
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                    <SyntaxHighlighter
                      ...
                      style={isDarkMode ? vscDarkPlus : prism}
                      showLineNumbers
                      customStyle={{ margin: 0, padding: '0.75rem', fontSize: '0.7rem', borderRadius: 0 }}
                    >
                      {generateCode()}
                    </SyntaxHighlighter>
                  </motion.div>
          ...
          <div className="flex justify-center my-6 md:hidden">
            <button
              ...
              className={`relative px-5 py-2 flex items-center justify-center gap-2 transition-all duration-200 rounded-md bg-black border border-black dark:border-gray-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 ${
                activeTab === 'code'
                  ? 'text-gray-300'
